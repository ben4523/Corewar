...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/12/21 15:45:02 by unli-yaw          #+#    #+#             */
/*   Updated: 2017/12/21 15:45:04 by unli-yaw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <corewar.h>

int				octet_cod_there(t_data *data, t_listb *elem)
{
...
char			*get_par_types_no_ocp(t_data *data, t_listb *elem)
{
...
int				octet_cod(t_data *data, t_listb *elem)
{
	if (data->map[elem->pc] == 0x02 || data->map[elem->pc] == 0x03 || \
			data->map[elem->pc] == 0x04 || data->map[elem->pc] == 0x05 || \
			data->map[elem->pc] == 0x06 || data->map[elem->pc] == 0x07 || \
...
unsigned int	*get_params(unsigned int *par_types,
							t_data *data, t_listb *elem)
{
	unsigned int	*params;
	int				cpt[2];
